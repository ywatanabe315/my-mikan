...
namespace {
  ...
  WithError<size_t> SetupPageMap(PageMapEntry* page_map,
    int page_map_level, LinerAddress4Level addr, size_t num_4kpages) {
  ...
  Error SetupPageMaps(LinerAddress4Level addr, size_t num_4kpages) {
  ...
  Error CleanPageMaps(LinerAddress4Level addr) {
    auto pml4_table = reinterpret_cast<PageMapEntry*>(GetCR3());
...
Error Terminal::ExecuteFile(const fat::DirectoryEntry& file_entry, char* command, char* first_arg) {
  ...
  LinerAddress4Level args_frame_addr{0xffff'ffff'ffff'f000};
  ...
  LinerAddress4Level stack_frame_addr{0xffff'ffff'ffff'e000};
  ...
  if (auto err = CleanPageMaps(LinerAddress4Level{addr_first})) {
    return err;
  }
